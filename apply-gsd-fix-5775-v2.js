...
console.log('GSD Fix for Issue #5775 (v2)\n');
...
const knownTypesPattern = /(\s*"rewrite-docs",)\n(\s*"workflow-preferences",)/;
const knownTypesReplacement = `$1\n  // Sidecar units (triage, quick-task)\n  "triage-captures",\n  "quick-task",\n$2`;

if (!manifestContent.includes('"triage-captures"')) {
  manifestContent = manifestContent.replace(knownTypesPattern, knownTypesReplacement);
  if (manifestContent.includes('"triage-captures"')) {
    console.log('✓ Added triage-captures and quick-task to KNOWN_UNIT_TYPES');
  } else {
    console.error('Error: Failed to add to KNOWN_UNIT_TYPES');
    process.exit(1);
  }
} else {
...
// Find "research-project": entry and insert after it
const researchProjectPattern = /(  "research-project": \{[^}]+maxSystemPromptChars: COMMON_BUDGET_MEDIUM,?\s*\},)/;
const match = researchProjectPattern.exec(manifestContent);
...
if (match && !manifestContent.includes('"triage-captures":')) {
  ...
  console.error('Error: Could not find "research-project" entry to insert after');
  process.exit(1);
}
...
if (oldFunction.test(phasesContent)) {
  ...
  console.error('Looking for pattern: unitWritesSource');
  console.error('Current function content:');
  const funcMatch = phasesContent.match(/function unitWritesSource[^}]+\}/s);
  if (funcMatch) {
    console.error(funcMatch[0]);
  }
  process.exit(1);
}
