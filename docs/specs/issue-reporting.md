...
- One capture directory holds **exactly one run**; concurrent capture runs are NOT possible. Entry atomically claims the directory (an O_CREAT|O_EXCL claim file, `capture_run.claim`, created before any logging); any second start against a claimed or non-empty directory is rejected before a single record is written (exit code 2). Owner decision 2026-09-07 (PR #121 approval): this is an invariant of the contract, not a tolerated state — mixed-run evidence and false clean markers are corruption by definition and the design refuses to produce them.
