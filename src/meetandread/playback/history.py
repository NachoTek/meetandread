...
class HistoryPlaybackController:
    ...
    def load_transcript_audio(self, md_path: Optional[Path]) -> None:
        ...
        if not wav_path.exists() or not wav_path.is_file():
            ...
            logger.debug(
                "load_resolve_missing: stem=%s wav=%s",
                md_path.stem,
                wav_path.name,
            )
            logger.warning(
        ...
        logger.debug(
            "load_source_set: stem=%s wav=%s media_status=%s",
            md_path.stem,
            wav_path.name,
    ...
    def seek_to(self, position_ms: int) -> None:
        ...
        if duration > 0:
            ...
            if clamped_ms != position_ms:
                logger.debug(
                    "seek_clamped: requested_ms=%d clamped_ms=%d duration=%d stem=%s",
                    position_ms,
                    clamped_ms,
                    duration,
                    self._stem_or_none(),
    ...
    def _on_media_status_changed(
        ...
        if status == QMediaPlayer.MediaStatus.InvalidMedia:
            ...
            logger.debug(
                "media_end_of_audio: stem=%s position_ms=%d duration_ms=%d",
                self._stem_or_none(),
                self._player.position(),
    ...
    def _on_playback_state_changed(
        ...
        logger.debug(
            "playback_state_changed: state=%s stem=%s",
            state,
            self._stem_or_none(),
        )
