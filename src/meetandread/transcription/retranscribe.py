...
Sidecar naming convention::
    ...
    transcripts/{stem}_retranscribe_{model}.md
...
class RetranscribeRunner:
    ...
    on_complete:
        ...
        re-transcription finishes (either successfully or with an error).
    ...
    def __init__(
        ...
        # Background thread handle (one re-transcription at a time)
    ...
    def retranscribe_recording(
        ...
        """Start a background re-transcription.  Returns the expected sidecar path.
        ...
        self._thread = threading.Thread(
            target=self._run_retranscribe,
            args=(audio_path, sidecar_path, model_size, transcript_path),
            daemon=True,
            name="RetranscribeWorker",
        )
        self._thread.start()
        return str(sidecar_path)

    def cancel(self) -> None:
        """Request cancellation of the current re-transcription."""
    ...
    @classmethod
    def sidecar_path(cls, transcript_path: Path, model_size: str) -> Path:
    ...
    def reject_retranscribe(cls, transcript_path: Path, model_size: str) -> None:
        ...
        if sidecar.exists():
            sidecar.unlink()
            logger.info(
                "Rejected %s; deleted sidecar: %s", cls.SIDECAR_TAG, sidecar
            )
        else:
    ...
    @classmethod
    def _sidecar_path(cls, transcript_path: Path, model_size: str) -> Path:
        """Derive sidecar path: ``{stem}_{SIDECAR_TAG}_{model}.md``."""
        stem = transcript_path.stem  # e.g. "recording-2026-02-01-143045"
        return transcript_path.parent / f"{stem}_{cls.SIDECAR_TAG}_{model_size}.md"
    ...
    def _preserve_recording_time(
        ...
        on the new store so the real recording date survives re-transcription.
        """
    ...
    def _get_or_create_engine(
        ...
        with self._engines_lock:
            if model_size not in self._engines:
                logger.info(
                    "Creating %s engine for model %s",
                    self.SIDECAR_TAG, model_size,
                )
    ...
    def _run_retranscribe(
        ...
        """Execute the re-transcription in a background thread."""
        ...
        try:
            ...
            if self._cancel_event.is_set():
                logger.info("%s cancelled before engine creation", self.SIDECAR_TAG.capitalize())
                return
            ...
            try:
                ...
                logger.warning(
                    "Speaker identification during %s failed (non-fatal): %s",
                    self.SIDECAR_TAG, diar_exc,
                )
            ...
            logger.info("%s complete; sidecar written to %s", self.SIDECAR_TAG.capitalize(), sidecar_path)

        except Exception as exc:
            error = str(exc)
            logger.error("%s failed: %s", self.SIDECAR_TAG.capitalize(), exc)
    ...
    def _run_speaker_identification(
        ...
        _run_diarization_for_postprocess but adapted for the re-transcription
        context. Gracefully degrades — if diarization fails, returns the store
        ...
        logger.info(
            "%s diarization: %d segments, %d speakers",
            self.SIDECAR_TAG.capitalize(), len(result.segments), result.num_speakers,
        )
        ...
        logger.info(
            "%s speaker identification complete: %d/%d words tagged, "
            "%d speakers (%d matched)",
            self.SIDECAR_TAG.capitalize(), tagged_count, len(words),
            len(label_map), len(result.matches),
        )
